...
// ompnn - pybind11 module `_ompnn`.
//
// The binding layer is identical in ompnn, cudann and ompnn apart from the
...
#if __has_include("ompnn/build_info.hpp")
#include "ompnn/build_info.hpp"
#endif
#include <omp.h>
...
#include "ompnn/network.hpp"

namespace py = pybind11;
using namespace ompnn;

#ifndef OMPNN_MODULE_NAME
#define OMPNN_MODULE_NAME _ompnn
#endif
#ifndef OMPNN_PY_PACKAGE
#define OMPNN_PY_PACKAGE "ompnn"
#endif
#ifndef OMPNN_COMPILER_ID
#define OMPNN_COMPILER_ID "unknown"
#endif
#ifndef OMPNN_COMPILE_FLAGS
#define OMPNN_COMPILE_FLAGS ""
#endif
#ifndef OMPNN_SYCL_TARGETS
#define OMPNN_SYCL_TARGETS ""
#endif
#ifndef OMPNN_TARGET_NAME
#define OMPNN_TARGET_NAME "cpu"
#endif
#ifndef OMPNN_ONEMATH_VERSION
#define OMPNN_ONEMATH_VERSION ""
#endif
#ifndef OMPNN_GIT_SHA
#define OMPNN_GIT_SHA "unknown"
#endif
...
PYBIND11_MODULE(OMPNN_MODULE_NAME, m) {
    m.doc() = "ompnn: OpenMP feed-forward neural network (CBLAS / cuBLAS / rocBLAS + OpenMP kernels), 1:1 port of syclnn";
    m.attr("__version__") = OMPNN_VERSION_STRING;
    ...
    auto opts = py::class_<Options>(m, "Options", "Runtime options: device, BLAS backend, profiling, ablation switches.")
                    ...
                    .def_readwrite("blas_queue", &Options::blas_queue)
                    .def_readwrite("sync_ops", &Options::sync_ops);
    ...
    m.def(
        ...
        "The OpenMP host and every offload device, as dicts (index, name, type, backend, ...).");
    m.def(
        "get_omp_devices",
        [] {
        ...
        "Names of all visible devices (syclnn.get_sycl_devices counterpart).");
    m.def(
        "build_info",
        [] {
            py::dict d;
            d["version"] = OMPNN_VERSION_STRING;
            d["compiler"] = OMPNN_COMPILER_ID;
            d["flags"] = OMPNN_COMPILE_FLAGS;
            d["offload_arch"] = OMPNN_SYCL_TARGETS;
            d["onemath"] = OMPNN_ONEMATH_VERSION;
            d["git_sha"] = OMPNN_GIT_SHA;
            d["blas_backends"] = compiled_blas_backends();
            d["blas_selectable"] = compiled_blas_backends();
            d["openmp"] = _OPENMP;
            d["omp_num_devices"] = omp_get_num_devices();
            d["omp_max_threads"] = omp_get_max_threads();
            d["target"] = OMPNN_TARGET_NAME;
            d["cblas"] = OMPNN_BLAS_NAME;
